...
1. If you are uncertain were a specific block "inserts" their content  just by reading the examples, open up ("**base-main.html**") and look for that block with the same name and see where it is placed!

   **There cannot be 2 blocks with the same name!**

2. For those unfamiliar with Jinja, Jinja blocks allow you to name the "closing block", but of course the name can ONLY be the same as the "opening block"!
...
- #### **2.2 Custom variables**
  ...
  - *2.2.1* Variable to **set the href link for the "skip navigation" button**

    If you decide to **KEEP** the default navigation bar you musn't forget to:  
    \- set up your nav buttons (More info below)  
  ...
  - *2.2.2* Variable to **disable the default navigation completely**
    ...
    \- remove the shylesheet link for the default navigation bar in `<head>`
...
- #### **2.3 List of all the EMPTY blocks**
  ...
  - *2.3.1* Block "stylesheet_extra"

    **It is intended that every page that "extends" the base template page has its
    own seperate CSS file, so that we keep things organized.**
  ...
  - *2.3.2* Block "page_description" (OPTIONAL)
  ...
  - *2.3.4* Attributes for `<header>` and `<footer>`

    The "attribute" blocks are used to only add attributes to the tag without needing to rewrite all of it if you only need to change the atrributes.
  ...
  - *2.3.5* Block "nav_extra_buttons"
    ...
    **RECOMMENDED:**
    No more than 2 buttons with not so many characters  in them as it will cause overflow!
  ...
  - *2.3.9* Block "footer_title"

    **By default the footer title is the SAME as the page title defined in the "title" block.**
  ...
  - *2.3.11* Block "footer_extra"
    ...
    **This Jinja block is placed UNDER the `<footer>` and ABOVE the `<scrip>` tags**
...
- #### **2.4 List of all the FULL blocks**
  ...
  *These blocks already have html written in them so if you call them you will essentially **erase** all of their original contents!*
  ...
  But of course if you desire to completely rework them and all of their content the option is there.
